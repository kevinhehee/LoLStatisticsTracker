    ...
    .then((response) => {
      ...
      for (let i = 0; i < allchamps.length; i++) {
        ...
        if (check.key === champ1.toString()) {
          returnthis.champ1ID = check.id;
          returnthis.champ1Name = check.name;
          break;
        }
      }
      ...
      for (let i = 0; i < allchamps.length; i++) {
        ...
        if (check.key === champ2.toString()) {
          returnthis.champ2ID = check.id;
          returnthis.champ2Name = check.name;
          break;
        }
      }
      ...
      for (let i = 0; i < allchamps.length; i++) {
        ...
        if (check.key === champ3.toString()) {
          returnthis.champ3ID = check.id;
          returnthis.champ3Name = check.name;
          break;
        }
      }
